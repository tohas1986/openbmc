...
cp -rp init shutdown update copyflash whitelist bin sbin usr lib etc var run/initramfs
...
# Look for the variable ($1) in u-boot environment.
# Return 0 if it's there and 1 if not
search_fw_env_var() {
	# do we have 1 or 2 copies of the environment?
	# count non-blank non-comment lines
	# copies=$(grep -v ^# /etc/fw_env.config | grep -c [::alnum::])
...
then
	...
	get_fw_env_var debug-init-sh >> $optfile
fi

debug_init_sh_end=n
search_fw_env_var debug-init-sh-end && debug_init_sh_end=y

debug_init_sh=n
search_fw_env_var debug-init-sh && debug_init_sh=y

echo rofs = $rofs $rofst   rwfs = $rwfs $rwfst

if test "x$debug_init_sh" = xy
then
...
# Check rofs signature
ro_check="FAIL"
ro_msg=""
pubkey=/etc/publickey

while test "$rwfst"  = "jffs2" -a -f $rwdir/rofs_hash.txt -a -f $rwdir/rofs_hash.sig -a -x /usr/bin/openssl
do
	echo "Proceeding ROFS check"
	echo -n "Verifying signature... "
	openssl dgst -sha256 -verify $pubkey -signature $rwdir/rofs_hash.sig $rwdir/rofs_hash.txt
	if test $? -ne 0
	then
		ro_check="FAIL"
		ro_msg="Failed to check signature"
		break
	fi

	# Now rofs_hash.txt is trusted
	size_file=$(tail -1 $rwdir/rofs_hash.txt | awk '{print $1}')
	# To speed up reading, need to do it in 4k blocks (that is squashfs default"
	size_file=$((size_file/4096))
	hash_file=$(tail -1 $rwdir/rofs_hash.txt | awk '{print $2}')
	hash_calc=$(dd if=/dev/$rofs bs=4k count=$size_file 2>/dev/null | sha256sum -b | awk '{print $1}')

	if test "$hash_calc" == "$hash_file"
	then
		ro_check="PASS"
		ro_msg="ROFS check passed."
	else
		ro_check="FAIL"
		ro_msg="ROFS check failed."
	fi
	break
done
echo "ROFS check status: $ro_check, $ro_msg"

# Unpack persistent
#echo "Unpacking persistent configuration"
#pers=$(cat /proc/mtd  | grep persistent | awk '{print $1}' | sed -e 's/://g' | sed -e 's/mtd/mtdblock/g')
#if ls /dev/$pers >/dev/null 2>&1
#then
#	mkdir /p || :
#	mount -t jffs2 /dev/$pers /p >/dev/null || :
#	if ls /p/rw.tar > /dev/null 2>&1
#	then
#		tar -C ${rwdir}/cow/ -xf /p/rw.tar >/dev/null || :
#	else
#		echo "Unpack failed: no stored persistent file"
#	fi
#	umount /p || :
#	rmdir /p || :
#else
#	echo "Unpack failed: no persistent partition"
#fi

# Detect upgrade and clean up a bit after
if test ! -f ${rwdir}/cow/etc/upgraded.flg
then
	echo "Cleaning up after upgrade"
	rm -rf ${rwdir}/cow/etc/default/dropbear >/dev/null 2>&1 || :
	rm -rf ${rwdir}/cow/etc/systemd/network/* >/dev/null 2>&1 || :
	sed -i '/^ADMIN/s/\/bin\/sh/\/bin\/true/g' ${rwdir}/cow/etc/passwd
	touch ${rwdir}/cow/etc/upgraded.flg || :
fi

# Remove bin and sbin directories
echo "Removing bin/ sbin/ from RWFS"
find ${rwdir} -name "bin" -type d  -exec rm -rf {} \; || :
find ${rwdir} -name "sbin" -type d  -exec rm -rf {} \; || :
find ${rwdir} -name "*.so.*" -type f  -exec rm -rf {} \; || :

echo "Checking auth keys"
if test ! -f ${rwdir}/cow/home/root/.ssh/authorized_keys
then
	echo "Enabling password auth"
	echo 'DROPBEAR_EXTRA_ARGS="-B"' > ${rwdir}/cow/etc/default/dropbear
else
	echo "Disabling password auth"
	echo 'DROPBEAR_EXTRA_ARGS="-B -g"' > ${rwdir}/cow/etc/default/dropbear
fi
...
if grep -w factory-reset $optfile
then
	echo "Factory reset: restoring factory RWFS."
	rm -rf $upper/* >/dev/null 2>&1 || :
	tar xzf /$rodir/usr/share/rwfs/rwfs.tgz -C $upper/ || :
	[ -e "/dev/$persistent" ] && echo "Clearing persistent storage." && flash_eraseall /dev/$persistent
fi
...
if test "x$debug_init_sh_end" = xy
then
	debug_takeover "Debug initial shell at the very end requested by command line."
fi
