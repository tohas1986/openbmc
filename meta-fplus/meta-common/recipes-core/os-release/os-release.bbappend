...
def run_arc(d, cmd):
    try:
        oeroot = d.getVar('OS_RELEASE_ROOTPATH', True)
        return bb.process.run(("export PSEUDO_DISABLED=1; " +
                               "rev=$(arc --work-tree $(dirname $(dirname %s)) --arc-dir $(dirname $(dirname %s))/.arc info | grep revision | awk '{print $2}'); " + 
                                "hash=$(arc --work-tree $(dirname $(dirname %s)) --arc-dir $(dirname $(dirname %s))/.arc info | grep hash | awk '{print $2}'); " + 
                                "if [ -n \"$rev\" ]; then echo $rev; else echo $hash; fi ")
            % (oeroot, oeroot, oeroot, oeroot))[0].strip('\n')
    except Exception as e:
        bb.warn("Unexpected exception from 'arc' call: %s" % e)
        pass
...
PHOSPHOR_OS_RELEASE_DISTRO_VERSION := "${@run_arc(d, 'rev-parse HEAD')}"
