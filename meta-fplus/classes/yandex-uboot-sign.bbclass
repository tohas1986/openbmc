...
python() {
    ...
    if d.getVar('UBOOT_FITIMAGE_ENABLE') == '1' or sign:
        ...
        d.appendVar('DEPENDS', " u-boot-fw-utils-aspeed-sdk dtc-native")
    if sign:
...
concat_dtb() {
	...
	bbwarn "Yandex debug: concat_dtb"
	if [ -e "${UBOOT_DTB_BINARY}" ]; then
		# Re-sign the kernel in order to add the keys to our dtb
		bbwarn "Yandex debug: re-sign the kernel"
		bbwarn "${UBOOT_MKIMAGE_SIGN} -F -k ${UBOOT_SIGN_KEYDIR} -K ${UBOOT_DTB_BINARY} -r ${B}/fitImage-linux ${UBOOT_MKIMAGE_SIGN_ARGS}"
		${UBOOT_MKIMAGE_SIGN} \
...
deploy_dtb() {
	...
	bbwarn "Yandex debug: deploy_dtb"
	if [ -n "${type}" ]; then
...
concat_spl_dtb() {
	bbwarn "Yandex debug: concat_spl_dtb"
...
deploy_spl_dtb() {
	...
	bbwarn "Yandex debug: deploy_spl_dtb"
	if [ -n "${type}" ]; then
...
do_uboot_generate_rsa_keys() {
	bbwarn "Yandex debug: uboot_generate_rsa_keys"
	...
	if [ "${SPL_SIGN_ENABLE}" = "1" ] && [ "${UBOOT_FIT_GENERATE_KEYS}" = "1" ]; then
		...
		if [ ! -f "${SPL_SIGN_KEYDIR}/${SPL_SIGN_KEYNAME}".key ] || \
			...
			bbwarn "Yandex debug: generate SPL sign keys"
...
uboot_fitimage_assemble() {
	bbwarn "Yandex debug: uboot_fitimage_assemble"
...
EOF
	...
	#
	bbwarn "Yandex debug: Compiling ITS to DTB"
	bbwarn "Yandex debug: ${UBOOT_MKIMAGE} ${SPL_MKIMAGE_DTCOPTS} -f ${UBOOT_ITS} ${UBOOT_FITIMAGE_BINARY}"
	${UBOOT_MKIMAGE} \
	...
	if [ "${SPL_SIGN_ENABLE}" = "1" ] ; then
		...
		#
		bbwarn "Yandex debug: Signing U-Boot FIT Image"
		bbwarn "${UBOOT_MKIMAGE_SIGN} ${SPL_MKIMAGE_DTCOPTS} -F -k ${SPL_SIGN_KEYDIR} -K ${SPL_DIR}/${SPL_DTB_BINARY} -r ${UBOOT_FITIMAGE_BINARY} ${SPL_MKIMAGE_SIGN_ARGS}"
		${UBOOT_MKIMAGE_SIGN} \
			${@'-D "${SPL_MKIMAGE_DTCOPTS}"' if len('${SPL_MKIMAGE_DTCOPTS}') else ''} \
			-F -k "${SPL_SIGN_KEYDIR}" \
	...
	bbwarn "Signed SPL DTB: ${SPL_DIR}/${SPL_DTB_SIGNED}"
}
...
uboot_assemble_fitimage_helper() {
	...
	bbwarn "Yandex debug: uboot_assemble_fitimage_helper type=${type} binary=${binary}"

	if [ "${UBOOT_SIGN_ENABLE}" = "1" -a -n "${UBOOT_DTB_BINARY}" ] ; then
		bbwarn "Yandex debug: to call concat_dtb $type $binary"
		concat_dtb $type $binary
	fi

	if [ "${UBOOT_FITIMAGE_ENABLE}" = "1" -a -n "${SPL_DTB_BINARY}" ]; then
		bbwarn "Yandex debug: to call uboot_fitimage_assemble"
		uboot_fitimage_assemble
	fi

	if [ "${SPL_SIGN_ENABLE}" = "1" -a -n "${SPL_DTB_BINARY}" ] ; then
		bbwarn "Yandex debug: to call concat_spl_dtb"
		concat_spl_dtb
	fi
}

do_uboot_assemble_fitimage() {
	bbwarn "Yandex debug: do_uboot_assemble_fitimage UBOOT_CONFIG: ${UBOOT_CONFIG}"
	...
	if [ -n "${UBOOT_CONFIG}" ]; then
		...
		for config in ${UBOOT_MACHINE}; do
			...
			cd ${B}/${config}
			bbwarn "Yandex debug: do_uboot_assemble_fitimage type=${type}"
...
deploy_helper() {
	...
	bbwarn "Yandex debug: deploy_helper"
...
do_deploy:prepend() {
	bbwarn "Yandex debug: deploy:prepend"
	if [ -n "${UBOOT_CONFIG}" ]; then
		unset i j k
		for config in ${UBOOT_MACHINE}; do
			i=$(expr $i + 1);
...
do_deploy:append() {
	bbwarn "Yandex debug: deploy:append"
