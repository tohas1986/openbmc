...
do_otptool() {
	...
	bbwarn "Yandex debug: otptool make_otp_image --key_folder ${OTPTOOL_KEY_DIR} --output_folder ${otptool_config_outdir} --no_last --mark_16th_key ${otptool_config} ${OTPTOOL_EXTRA_OPTS}"
    otptool make_otp_image \
        --key_folder ${OTPTOOL_KEY_DIR} \
        --output_folder "${otptool_config_outdir}" \
		--no_last --mark_16th_key \
        ${otptool_config} \
        ${OTPTOOL_EXTRA_OPTS}
