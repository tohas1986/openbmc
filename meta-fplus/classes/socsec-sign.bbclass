...
sign_spl_helper() {
    ...
    if [ "${SOC_FAMILY}" != "aspeed-g6" ] ; then
		...
		bbwarn "Yandex debug: socsec make_secure_bl1_image --soc ${SOCSEC_SIGN_SOC}  --rsa_sign_key ${SOCSEC_SIGN_KEY} --bl1_image ${DEPLOYDIR}/${SPL_IMAGE} $signing_helper_args ${SOCSEC_SIGN_EXTRA_OPTS} --output ${SPL_BINARY}.staged"
